function GetHVPlots(global_fontsize, global_subfontsize, global_markersize, global_primeLine, global_seconLine, global_imagesize)
    %% Tidy up
    close all; clc;
    ...
    lineColor = [0.75 0.775 0.9];

    TestSetting = TestSettings();

    probNumber = 0;

    for exp = ["STD", "HARDER", "HARDEST"]
        for prob = TestSetting.problemClasses
            for v = prob.versions
            %% Tidy up
            close all;
                %% Get All HV in one Array
                filenames = append(prefix, suffix1, exp, suffix2, '*', prob.name, string(v), '*');
                ...
                for i = 1:length(TestSetting.algorithms)
                    ...
                    results(probNumber*length(TestSetting.algorithms) + i,:)= ...
                        struct('problem', prob.name, 'version', v, 'algorithm', ...
                        alg(1), 'data', ...
                        load(append(prefix, files_for_p(i))).res);
                end

                %% Hypervolume over time for single algorithms   
                 pro = str2func(append(prob.name, string(v)));
                 p = pro();
                 if(exp == "STD")
                     M = p.M;
                     D = p.D;
                 ...
                 t = append(prob.name, string(v), ", M = ", string(M), ", D = ", string(D));
                ...
                ylabel('HV','fontsize',global_fontsize);

                singleResults = struct('Algorithm',{},'FEs',{},'FullHV',{},'LastHV',{});
                hold on
                grid on

                for i = 1:length(TestSetting.singleAlgorithms)
                    data = results(...
                    join([results.problem; results.version],"",1) == append(prob.name, string(v))...
                    ...
                    singleResults(i) = ...
                        ...
                        'FullHV',  data.data.median_run.metric.HV, ...
                        'LastHV', data.data.median_run.metric.HV(end));

                    p = plot(singleResults(i).FEs, singleResults(i).FullHV,...
                        marker(i),'LineWidth', global_primeLine);
                    p.MarkerSize = global_markersize;
                    p.MarkerFaceColor = lineColor.*(i/length(TestSetting.singleAlgorithms));
                    p.Color = lineColor.*(i/length(TestSetting.singleAlgorithms));
                end
                ...
                exportgraphics(f1, 'EAXOpt/Images/Plots/' + name(2) ...
                ...
                for i = 1:length(TestSetting.hhAlgorithms)
                    ...
                    p = plot([data.data.median_run.result{:,1}],...
                        data.data.median_run.metric.HV,...
                        marker(i),'LineWidth', global_primeLine);
                ...
                [~,idx] = max([singleResults.LastHV]);

                p = plot(singleResults(idx).FEs,singleResults(idx).FullHV, ...  
                ...
                exportgraphics(f2, 'EAXOpt/Images/Plots/' + name(2) ...
                ...
                for i = 1:length(TestSetting.algorithms)
                    ...
                    y(i) = struct('name', n, ... 
                        'endResult', data.median_run.metric.HV(end), ...
                        'endIQR', data.iqrEnd, ...
                        'midResult', data.median_run.metric.HV(6), ...
                        'midIQR', data.iqrMid);
                end
                ...
                rectangle('Position',[0  min([singleResults.LastHV])...
                    length(x)+1 max([singleResults.LastHV])-min([singleResults.LastHV]),], ...
                ...
                set(gca,'FontSize', global_fontsize/1.5) 
                ylabel('HV','fontsize', global_fontsize/1.5);

                grid on
                hold off
                
                title(t, 'fontsize', global_fontsize/1.5);
                
                plo = get(gca,'Position');
                set(gca,'Position',[plo(1) plo(2)+0.1 plo(3)-0.2 plo(4)-0.2]);

                exportgraphics(f3, 'EAXOpt/Images/Plots/' + name(2) + '_' ... 
